...
describe('StorageManagerService', () => {
  ...
  describe('getAvailableDrives', () => {
    it('should return list of available drives', async () => {
      ...
      expect(exec).toHaveBeenCalledWith('wmic logicaldisk get name', expect.any(Function));
    });
    ...
    it('should handle exec errors', async () => {
      ...
      await expect(storageManagerService.getAvailableDrives()).rejects.toThrow('WMIC failed');
    });
  });
  ...
  describe('getDriveInfo', () => {
    ...
    it('should handle exec errors', async () => {
      ...
      await expect(storageManagerService.getDriveInfo('Z:\\')).rejects.toThrow('Drive not found');
    });
  });
  ...
  describe('scanDirectory', () => {
    beforeEach(() => {
      ...
      fs.readdir
        .mockResolvedValueOnce([
          { name: 'large-file.txt', isFile: () => true, isDirectory: () => false },
          { name: 'small-file.txt', isFile: () => true, isDirectory: () => false },
          { name: 'large-folder', isFile: () => false, isDirectory: () => true },
        ])
    ...
    it('should handle scan errors', async () => {
      ...
      await expect(storageManagerService.scanDirectory('C:\\restricted')).rejects.toThrow(
        'Access denied'
      );
    });
  });
  ...
  describe('calculateFolderSize', () => {
    it('should calculate folder size recursively', async () => {
      ...
      fs.stat.mockResolvedValueOnce({ size: 1000 }).mockResolvedValueOnce({ size: 0 }); // subdir
      ...
      const result = await storageManagerService.calculateFolderSize('C:\\test');
    ...
    it('should handle inaccessible files gracefully', async () => {
      fs.readdir.mockResolvedValue([
        { name: 'accessible.txt', isFile: () => true, isDirectory: () => false },
        { name: 'inaccessible.txt', isFile: () => true, isDirectory: () => false },
      ]);
  ...
  describe('deleteItems', () => {
    it('should delete files and folders', async () => {
      const items = [{ path: 'C:\\temp\\file.txt' }, { path: 'C:\\temp\\folder' }];

      fs.stat
        .mockResolvedValueOnce({ size: 1000, isFile: () => true, isDirectory: () => false })
        .mockResolvedValueOnce({ size: 2000, isFile: () => false, isDirectory: () => true });
      ...
      expect(fs.rm).toHaveBeenCalledWith('C:\\temp\\folder', { recursive: true, force: true });
    });
