...
describe("PackageManagerService", () => {
  ...
  describe("isWingetInstalled", () => {
    test("should return true if winget is installed", async () => {
      jest
        .spyOn(packageManagerService, "isCommandInstalled")
        .mockResolvedValueOnce(true);
      expect(await packageManagerService.isWingetInstalled()).toBe(true);
      expect(packageManagerService.isCommandInstalled).toHaveBeenCalledWith(
        "winget",
      );
    });
  });
  ...
  describe("isPipInstalled", () => {
    ...
    test("should return false if no pip command is found", async () => {
      runProcessSpy.mockResolvedValue({
        code: 1,
        stdout: "",
        stderr: "not found",
      });
  ...
  describe("update commands", () => {
    test("updateWinget should call runUpdateCommand with correct args", async () => {
      ...
      expect(runUpdateCommandSpy).toHaveBeenCalledWith("winget", [
        "update",
        "--include-unknown",
      ]);
    });
    ...
    test("updateChoco should call runUpdateCommand with correct args", async () => {
      ...
      expect(runUpdateCommandSpy).toHaveBeenCalledWith("choco", [
        "upgrade",
        "all",
        "-y",
      ]);
  ...
  describe("updatePip", () => {
    test("should update pip and then outdated packages", async () => {
      ...
      jest
        .spyOn(packageManagerService, "isPipInstalled")
      ...
      runProcessSpy
        ...
        .mockResolvedValueOnce({
          code: 0,
          stdout: "packageA updated",
          stderr: "",
        }) // 4. update packageA
        .mockResolvedValueOnce({
          code: 0,
          stdout: "packageB updated",
          stderr: "",
        }); // 5. update packageB
      ...
      expect(runProcessSpy).toHaveBeenCalledWith("pip", [
        "install",
        "--upgrade",
        "pip",
      ]);
    ...
    test("should log error if updating a specific pip package fails", async () => {
      ...
      runProcessSpy
        ...
        .mockResolvedValueOnce({
          code: 1,
          stdout: "",
          stderr: "packageA update failed",
        }); // 4. pip install --upgrade packageA fails

      await packageManagerService.updatePip();

      expect(mockLogService.log).toHaveBeenCalledWith(
        "Error updating pip. Exit Code: 1",
      );
    });
    ...
    test("should throw error if no pip installation is found", async () => {
      ...
      await expect(packageManagerService.updatePip()).rejects.toThrow(
        "No pip installation found",
      );
    });
  });
  ...
  describe("updatePowerShellModules", () => {
    test("should update PowerShell modules successfully", async () => {
      ...
      expect(runProcessSpy).toHaveBeenCalledWith("pwsh", [
        "Update-PSResource",
        "-Force",
      ]);
      expect(mockLogService.log).not.toHaveBeenCalledWith(
    ...
    test("should log error if Update-Module fails", async () => {
      runProcessSpy.mockRejectedValueOnce(
        new Error("Update-Module process error"),
      ); // Update-Module fails
    ...
    test("should log error if Update-PSResource fails", async () => {
      runProcessSpy
        .mockResolvedValueOnce({
          code: 0,
          stdout: "Update-Module success",
          stderr: "",
        }) // Update-Module succeeds
  ...
  describe("getAvailableUpdatesAsync", () => {
    beforeEach(() => {
      jest
        .spyOn(packageManagerService, "isWingetInstalled")
        .mockResolvedValue(false);
      jest
        .spyOn(packageManagerService, "isNpmInstalled")
        .mockResolvedValue(false);
      jest
        .spyOn(packageManagerService, "isScoopInstalled")
        .mockResolvedValue(false);
      jest
        .spyOn(packageManagerService, "isChocoInstalled")
        .mockResolvedValue(false);
      jest
        .spyOn(packageManagerService, "isPipInstalled")
        .mockResolvedValue(false);
      ...
      jest
        .spyOn(packageManagerService, "getWingetAvailableUpdatesAsync")
        .mockResolvedValue([]);
      jest
        .spyOn(packageManagerService, "getNpmAvailableUpdatesAsync")
        .mockResolvedValue([]);
      jest
        .spyOn(packageManagerService, "getScoopAvailableUpdatesAsync")
        .mockResolvedValue([]);
      jest
        .spyOn(packageManagerService, "getChocoAvailableUpdatesAsync")
        .mockResolvedValue([]);
      jest
        .spyOn(packageManagerService, "getPipAvailableUpdatesAsync")
        .mockResolvedValue([]);
    });
    ...
    test("should log errors if getting updates for a manager fails", async () => {
      ...
      expect(updates).toEqual([]);
      expect(mockLogService.log).toHaveBeenCalledWith(
        "Error getting winget updates: Winget error",
      );
    });
  });
...
PowerToys            Microsoft.PowerToys  0.51.1           0.53.0           winget`,
      ...
      const updates =
        await packageManagerService.getWingetAvailableUpdatesAsync();
      expect(updates).toEqual([
        {
...
git     2.34.1   2.35.1`,
      ...
      const updates =
        await packageManagerService.getScoopAvailableUpdatesAsync();
      expect(updates).toEqual([
        {
...
packageB|3.0.0|4.0.0|`,
      ...
      const updates =
        await packageManagerService.getChocoAvailableUpdatesAsync();
      expect(updates).toEqual([
        {
