  ...
  describe('runPingTest', () => {
    test('should return successful ping output for Windows', async () => {
      ...
      spawn.mockImplementation(() => ({
        stdout: {
          on: (event, callback) => {
            if (event === 'data') callback('Pinging 8.8.8.8 with 32 bytes of data:\r\nReply from 8.8.8.8: bytes=32 time=10ms TTL=118\r\n');
          },
        },
        stderr: { on: jest.fn() },
        on: (event, callback) => {
      ...
      expect(spawn).toHaveBeenCalledWith('ping', ['-n', '4', '-w', '5', '8.8.8.8'], expect.any(Object));
    ...
    test('should return successful ping output for non-Windows', async () => {
      ...
      spawn.mockImplementation(() => ({
        stdout: {
          on: (event, callback) => {
            if (event === 'data') callback('PING 8.8.8.8 (8.8.8.8): 56 data bytes\n64 bytes from 8.8.8.8: icmp_seq=0 ttl=118 time=10.000 ms\n');
          },
        },
        stderr: { on: jest.fn() },
        on: (event, callback) => {
      ...
      expect(spawn).toHaveBeenCalledWith('ping', ['-c', '4', '-W', '5', '8.8.8.8'], expect.any(Object));
    ...
    test('should log error if spawn throws an error', async () => {
      ...
      expect(mockLogService.log).toHaveBeenCalledWith('Error running ping test: Spawn failed');
    });
  });
  ...
  describe('runSpeedTest', () => {
    test('should return formatted speed test results on success', async () => {
      const mockSpeedtestOutput = {
        ...
        "isp": "Test ISP",
        "server": { "id": 1234, "name": "Test Server", "location": "New York", "country": "US", "host": "test.server.com" }
      };
      ...
      expect(spawn).toHaveBeenCalledWith('speedtest', ['--json'], expect.any(Object));
      expect(mockLogService.log).toHaveBeenCalledWith('Speed test completed successfully.');
    });
    ...
    test('should return error message on speedtest command failure', async () => {
      ...
      const result = await networkService.runSpeedTest();
      expect(result).toContain('Error running speed test: speedtest command not found');
      expect(mockLogService.log).toHaveBeenCalledWith('Speed test command failed with code 1: speedtest command not found');
    });
    ...
    test('should return error message on invalid JSON output', async () => {
      ...
      expect(mockLogService.log).toHaveBeenCalledWith(expect.stringContaining('Exception running speed test: Unexpected token'));
    });
    ...
    test('should log and return error if spawn throws an error', async () => {
      ...
      const result = await networkService.runSpeedTest();
      expect(result).toContain('Error running speed test: Speedtest spawn failed');
      expect(mockLogService.log).toHaveBeenCalledWith('Exception running speed test: Speedtest spawn failed');
    });
  });
});
