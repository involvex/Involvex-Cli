...
describe('DnsService', () => {
  ...
  describe('setDns', () => {
    test('should successfully set primary DNS', async () => {
      ...
      expect(mockLogService.log).toHaveBeenCalledWith(`Setting DNS to ${primaryDns}`);
      expect(mockLogService.log).toHaveBeenCalledWith('DNS settings updated successfully');
    });
    ...
    test('should return false and log error if primary DNS setting fails', async () => {
      const primaryDns = '1.1.1.1';
      runNetshCommandSpy.mockResolvedValueOnce({ code: 1, stdout: '', stderr: 'Error' });

      const result = await dnsService.setDns(primaryDns);

      expect(result).toBe(false);
      expect(mockLogService.log).toHaveBeenCalledWith('Failed to set primary DNS');
    ...
    test('should return false and log error if secondary DNS setting fails', async () => {
      ...
      expect(result).toBe(false);
      expect(mockLogService.log).toHaveBeenCalledWith('Failed to set secondary DNS');
      // No 'Error setting DNS' log here, as runNetshCommand did not throw an exception
    });
  ...
  describe('resetDns', () => {
    test('should successfully reset DNS', async () => {
      ...
      expect(result).toBe(true);
      expect(runNetshCommandSpy).toHaveBeenCalledWith('interface ip set dns name="Ethernet" dhcp');
      expect(mockLogService.log).toHaveBeenCalledWith('Resetting DNS to DHCP');
      expect(mockLogService.log).toHaveBeenCalledWith('DNS settings reset successfully');
    });
    ...
    test('should return false and log error if DNS resetting fails', async () => {
      ...
      expect(result).toBe(false);
      expect(mockLogService.log).toHaveBeenCalledWith('Failed to reset DNS settings');
  ...
  describe('runNetshCommand', () => {
    ...
    test('should resolve with stdout and stderr on close', async () => {
      ...
      const result = await dnsService.runNetshCommand('interface ip show config');

      expect(result).toEqual({ code: 0, stdout: 'netsh stdout', stderr: 'netsh stderr' });
      expect(mockLogService.log).toHaveBeenCalledWith('[netsh STDOUT]: netsh stdout');
      expect(mockLogService.log).toHaveBeenCalledWith('[netsh STDERR]: netsh stderr');
    });
    ...
    test('should reject on process error', async () => {
      ...
      await expect(dnsService.runNetshCommand('invalid command')).rejects.toThrow('Spawn error');
    });
  });
});
