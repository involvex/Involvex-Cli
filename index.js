        ...
        try {
            switch (selected) {
                ...
                case 'Update PowerShell Modules': {
                    await runPackageUpdate(screen, 'PowerShell Modules', () => packageManagerService.updatePowerShellModules());
                    break;
                }
                case 'Update All': {
...
async function showAvailableUpdates(screen) {
    ...
    try {
        ...
        if (updates.length === 0) {
            showMessage(screen, 'Available Updates', 'No updates available. All packages are up to date!');
        } else {
            ...
            showMessage(screen, 'Available Updates', `Found ${updates.length} available update(s):\n\n${updateText}`);
        }
    } catch (error) {
        progressDialog.stop();
        progressDialog.destroy();
...
async function runUpdateAll(screen) {
    ...
    try {
        ...
        if (availableManagers.length === 0) {
            progressDialog.stop();
            progressDialog.destroy();
            showMessage(screen, 'No Package Managers', 'No supported package managers found on this system.');
            return;
        }
...
async function showCreateRestorePointDialog(screen) {
    ...
    inputDialog.input('Enter description for restore point:', 'Manual System Restore Point', async (error, value) => {
        if (error) {
            inputDialog.destroy();
            screen.render();
            return;
...
async function showPluginsMenu(screen) {
    ...
    pluginsDialog.on('select', async (item, _index) => {
        ...
        try {
            switch (selected) {
                case 'List Installed Plugins': {
                    ...
                    const pluginNames = plugins.map(p => `${p.name || 'Unknown'} - ${p.description || 'No description'}`);
        ...
        if (!['Browse Plugin Repository', 'Install Plugin', 'Uninstall Plugin', 'Execute Plugin'].includes(selected)) {
...
async function showPluginRepositoryDialog(screen) {
    ...
    try {
        ...
        if (plugins.length === 0) {
            showMessage(screen, 'Plugin Repository', 'No plugins available in repository or unable to fetch repository.');
        } else {
...
async function showExecutePluginDialog(screen) {
    ...
    inputDialog.input('Enter plugin name to execute:', '', async (error, value) => {
        ...
        showMessage(screen, 'Plugin Execution', `Plugin '${value}' execution not yet implemented in this demo.`);
