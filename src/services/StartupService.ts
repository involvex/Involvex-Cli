...
export default class StartupService {
  ...
  async listStartupPrograms(): Promise<string[]> {
    this.logService.log(
      "Listing startup programs from registry and Task Scheduler.",
    );
    const startupPrograms: string[] = [];
    ...
    try {
      ...
      this.logService.log(
        `Exception listing startup programs: ${error instanceof Error ? error.message : String(error)}`,
      );
  ...
  async disableStartupProgram(programName: string): Promise<boolean> {
    this.logService.log(
      `Attempting to disable startup program: ${programName}`,
    );
    ...
    try {
      ...
      if (result.code === 0 && result.stdout.includes("SUCCESS")) {
        this.logService.log(
          `Successfully disabled startup program: ${programName}`,
        );
        return true;
      } else {
        this.logService.log(
      ...
      this.logService.log(
        `Exception disabling startup program ${programName}: ${error instanceof Error ? error.message : String(error)}`,
      );
      return false;
    }
  }
