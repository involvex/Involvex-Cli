...
export default class SystemRestoreService {
  ...
  async createRestorePoint(description: string): Promise<boolean> {
    try {
      ...
      this.logService.log(
        `Error creating restore point: ${error instanceof Error ? error.message : String(error)}`,
      );
      return false;
    }
  }

  async listRestorePoints(): Promise<string> {
    try {
      ...
      this.logService.log(
        `Error listing restore points: ${error instanceof Error ? error.message : String(error)}`,
      );
      return `Error: ${error instanceof Error ? error.message : String(error)}`;
    }
  }

  async deleteRestorePoint(sequenceNumber: number): Promise<boolean> {
    try {
      this.logService.log(
        `Deleting restore point with sequence: ${sequenceNumber}`,
      );
      ...
      this.logService.log(
        `Error deleting restore point: ${error instanceof Error ? error.message : String(error)}`,
      );
      return false;
    }
  }

  async deleteOldRestorePoints(keepCount: number): Promise<boolean> {
    try {
      this.logService.log(
        `Deleting old restore points, keeping ${keepCount} most recent`,
      );
      ...
      if (result.stdout.includes("No old restore points")) {
        ...
        this.logService.log(
          "Deleting old restore points not supported in this implementation",
        );
        return false;
      } else {
      ...
      this.logService.log(
        `Error deleting old restore points: ${error instanceof Error ? error.message : String(error)}`,
      );
      return false;
    }
  }
